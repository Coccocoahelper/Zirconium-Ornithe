...
public class CompactSineLUT {
    ...
    static {
        ...
        float[] sineArray = MathHelperAccessor.getSINE();

        // Copy the sine table, covering to raw int bits
        for (int i = 0; i < SINE_TABLE_INT.length; i++) {
            SINE_TABLE_INT[i] = Float.floatToRawIntBits(sineArray[i]);
        }

        SINE_TABLE_MIDPOINT = sineArray[sineArray.length / 2];

        // Test that the lookup table is correct during runtime
        /*for (int i = 0; i < sineArray.length; i++) {
            float expected = sineArray[i];
